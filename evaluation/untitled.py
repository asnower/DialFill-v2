...
def eval_acc(predictions, references, retrieved, knowledges):
    def lower(text):
    ...
    for pred, refe, retr, kg in zip(tqdm(predictions), references, retrieved, knowledges):
        pred_response = lower(pred)
        pred_knowledge = lower(retr)
        ...
        sent_acc.append(int(pred_knowledge in label_knowledge))
...
def eval_rag(predictions, retrieved_render, retrieved_obj=None):
    def lower(text):
        if isinstance(text, str):
            text = text.strip().lower()
            text = ' '.join(nltk.word_tokenize(text))
    ...
    if not retrieved_obj:
        retrieved_obj = retrieved_render

    k_f1 = []
    zero_k_f1 = []

    for pred, retr, kg in zip(tqdm(predictions), retrieved_render, retrieved_obj):
        pred_response = lower(pred)
        pred_knowledge = lower(retr)
        label_knowledge = lower(kg)
        ...
        if len(label_knowledge) == 0:
            ...
            k_f1.append(f1_score(pred_response, [label_knowledge]))
            k_score = f1_score(pred_response, [pred_knowledge])
        if len(label_knowledge) != 0:
    ...
    return {'RKF1': sum(k_f1) / len(k_f1) * 100,
            'ZRK': sum(zero_k_f1) / len(zero_k_f1) * 100}
...
def _eval_rouge(run, ref):
    ...
    return {'ROUGE_L_F1': rounder(rouge_l)}
...
def eval_bleu(predicts, answers):
    ...
    return {'BLEU4': rounder(bleu4 * 100 / len(_answers))}
